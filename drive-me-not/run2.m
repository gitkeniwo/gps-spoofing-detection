...
% T = 1; sd = spoofdect(); sd = sd.loadtrace(DS(T)); expF=25; sd = sd.estimatePathErr(expF, 0.9); sd = sd.run([], expF, true);  sd = sd.fp_analysis();
...
fig = figure(nfig); nfig = nfig + 1;
hold on;
plot(xb, 1 - cumsum(h), 'o-r', xb, 1 - cumsum(hb), 'o-b')
ylabel("1-CDF");
...
distw = rxdw(:, 1); distw = distw(distw < 0.315);
